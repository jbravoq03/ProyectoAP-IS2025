  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }}>
        <Heading size="md" className="text-black mb-4">Recursos más usados</Heading>
        {Object.entries(reservasPorRecurso).map(([idRecurso, cantidad]) => (
          <Text key={idRecurso} className="text-black"> Recurso: {recursos.find((r) => r.idRec === Number(idRecurso))?.nombre} → {cantidad} veces usado</Text>
        ))}
